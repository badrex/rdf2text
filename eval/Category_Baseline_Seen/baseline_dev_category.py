...
# put all files in each triple size in finalfiles ,i.e in the folder 1triple take all files and put them in finalfiles
    finalfiles = []
    for item in finaldirs:
        finalfiles += [(item, filename) for filename in os.listdir(item)]
# if category parameter is determined then take in each triple folder just the files that contains the specified category
    if category:
        finalfiles = []
        for item in finaldirs:
...
# create the files dev-webnlg-all-notdelex.triple | dev-webnlg-all-delex.triple  | dev-webnlg-all-notdelex.lex | dev-webnlg-all-delex.lex
    with open(folder+'/'+dataset + '-triple-'+ options + '.triple', 'w+', encoding="utf8") as f:
        f.write('\n'.join(source_out))
    with open(folder+'/'+dataset + '-triple-'+ options + '.lex', 'w+', encoding="utf8") as f:
        f.write('\n'.join(target_out))
...
def relexicalise(predfile,folder,rplc_list):
    """
    Take the file (predfile) from seq2seq output and write a relexicalised version of it.
    :param rplc_list: list of dictionaries of replacements for each example (UPPER:not delex item)
    :param folder the path of the folder to put the generated relex file in it
    ...
    with open(folder+'/'+'dev-triple-all-notdelex.triple', 'r', encoding="utf8") as f:
    ...
    for category in categories:
        ...
        for option in options:
            ...
            if option == 'all-delex':
                ...
                print('Total of {} files processed in {} with {} mode'.format(len(files), category, option))
...
def main(argv):
    usage = 'usage:\npython3 webnlg_baseline_dev_category_input.py -i <data-directory>' \
