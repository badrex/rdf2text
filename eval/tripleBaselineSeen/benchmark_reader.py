...
# give number of lex in each entry
    def count_lexs(self):
...
class Benchmark:
    ...
    def __init__(self):
        self.entries = []
#Et is elementTree is used to parse xml files 
# fileslist is tuple filePath/fileNAme
    def fill_benchmark(self, fileslist):
        for file in fileslist:
            tree = Et.parse(file[0] + '/' + file[1])
            root = tree.getroot()
            for xml_entry in root.iter('entry'):
                # knowing the end of current entry by lex tag
                lexfound = False
...
#Et is elementTree is used to parse xml files 
# fileslist is tuple filePath/fileNAme
    def fill_benchmark_file(self, fil):
        tree = Et.parse(fil)
        root = tree.getroot()
        for xml_entry in root.iter('entry'):
