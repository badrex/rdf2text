...
#            target_out.append(' '.join(out_trg.split()))
    ...
    if dataset == 'dev' and not delex:
        ...
        print('max ref length '+str(len(max_refs)))
        keys = [key for (key, value) in sorted(scr_refs.items())]#order the dict scr_refs according to alphabet order
       # print('sorted not delex triple length '+str(len(keys)))#keys list contain ordered not delex triple
        values = [value for (key, value) in sorted(scr_refs.items())]#values list  contain ordered not delex lex
        print('sorted not delex target length '+str(len(values)))
...
#for each line in baseline_pred.txt, put it in "pred" and its position  put it in "i"
    for i, pred in enumerate(predictions):
        # replace each item in the corresponding example
        rplc_dict = rplc_list[i]
        relex_pred = pred
        for key in rplc_dict:
#for each key replace its value i.e semantic type with the key
    ...
    with open('dev-webnlg-all-notdelex.triple', 'r', encoding="utf8") as f:#delete all whitespace with strip
